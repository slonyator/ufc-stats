...
class UFCStatsScraper:
    ...
    def get_meta_information(self):
        meta_info = {}
        fighters = self.soup.find_all('h3', class_='b-fight-details__person-name')
        meta_info['fighters'] = [fighter.get_text().strip() for fighter in fighters]

        # Extracting the method of victory, round, time, time format, and referee
        fight_details = self.soup.find_all('i', class_='b-fight-details__text-item')
    ...
    def get_totals(self):
        ...
        potential_sections = self.soup.find_all(['section', 'div', 'span'])  # Including more tag types

        for section in potential_sections:
            if 'Totals' in section.get_text():
                table = section.find_next('table')
                header_row = table.find('tr').find_all('th')
                headers = [header.get_text().strip().lower().replace(' ', '_').replace('.', '') for header in
                           header_row]
    ...
    def get_significant_strikes(self):
        ...
        section = self.soup.find('p', string=lambda text: text and 'Significant Strikes' in text)

        if section:
            table = section.find_next('table')
            header_row = table.find('tr').find_all('th')
            headers = [header.get_text().strip().lower().replace(' ', '_').replace('.', '') for header in header_row]
            ...
            for row in rows:
                ...
                for idx, col in enumerate(columns[1:], start=1):  # Start from 1 to skip the fighter's name
                    header = headers[idx]
                    fighter_data[header] = col.get_text().strip()
    ...
    def _fetch_landed_by_target(self):
        data = {}
        section = self.soup.find('h4', string=lambda text: text and 'Landed by target' in text)

        if section:
            chart_rows = section.find_next_sibling('div').find_all('div', class_='b-fight-details__charts-row')

            for row in chart_rows:
                target = row.find('i', class_='b-fight-details__charts-row-title').get_text().strip()
                values = row.find_all('i', class_='b-fight-details__charts-num')
                if len(values) == 2:
                    data[target] = {
    ...
    def _fetch_landed_by_position(self):
        data = {}
        section = self.soup.find('h4', string=lambda text: text and 'Landed by position' in text)

        if section:
            chart_rows = section.find_next_sibling('div').find_all('div', class_='b-fight-details__charts-row')

            for row in chart_rows:
                position = row.find('i', class_='b-fight-details__charts-row-title').get_text().strip()
                values = row.find_all('i', class_='b-fight-details__charts-num')
                if len(values) == 2:
                    data[position] = {
                        'red_fighter': values[0].get_text().strip(),
    ...
    def _fetch_per_round(self):
        ...
        per_round_section = self.soup.find('a', class_='b-fight-details__collapse-link_rnd js-fight-collapse-link')
        if not per_round_section:
            return 'Per Round section not found.'

        # Find the table next to the 'Per Round' section
        table = per_round_section.find_next('table', class_='b-fight-details__table js-fight-table')
        if not table:
            return 'Per Round table not found.'

        rounds_data = []
        # Each round has its own thead with round number and a subsequent tbody with details
        round_headers = table.find_all('thead',
        ...
        for round_header in round_headers:
            ...
            for row in round_rows:
                ...
                round_data = {
                    ...
                    'significant_strike_percentage': columns[3].get_text(strip=True),
...
if __name__ == '__main__':
    ...
    scraper = UFCStatsScraper("http://ufcstats.com/fight-details/3fa8ee3fdc04fe36")
